...
class MockModels {
    ...
    func getNotificationDetail() -> NotificationDetail? {
        let notificationDetail = try? NotificationDetail("{\"_links\":{\"ackSync\":{\"href\":\"https://api.fit-pay.com/ackSync\"}, \"completeSync\":{\"href\":\"https://api.fit-pay.com/completeSync\"}, \"creditCard\":{\"href\":\"https://api.fit-pay.com/creditCards/\(someId)\"}, \"device\":{\"href\":\"https://api.fit-pay.com/devices/\(someId)\"}},\"type\": \"\(someType)\", \"syncId\":\"\(someId)\", \"deviceId\": \"\(someId)\", \"userId\": \"\(someId)\", \"clientId\": \"\(someId)\", \"creditCardId\": \"\(someId)\"}")
        expect(notificationDetail).toNot(beNil())
        return notificationDetail
    }
